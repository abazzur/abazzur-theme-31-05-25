...
// Adicione este código no arquivo theme.js ou crie um novo arquivo JS para isso
document.addEventListener('DOMContentLoaded', function() {
  const header = document.querySelector('.header');
  let headerHoverTimeout;
  
  // Quando o mouse entra no header
  header.addEventListener('mouseenter', function() {
    // Limpa qualquer timeout pendente
    clearTimeout(headerHoverTimeout);
    // Adiciona a classe imediatamente
  ...
  header.addEventListener('mouseleave', function() {
    ...
    }, 350); // Mesmo tempo do megamenu (1200ms)
  });
});
